...
class Engine:
    ...
    def __init__(self, eye, environment, frame, max_depth, num_thread):
        ...
        self._ambient_lights = [
            l for l in self.environment.lights if type(l) is AmbientLight
        ]
        self._point_lights = [
            l for l in self.environment.lights if type(l) is PointLight
        ]
        self._directional_lights = [
            l for l in self.environment.lights if type(l) is DirectionalLight
        ]
    ...
    def _render_range(self, start, end, rows):
        for h in range(start, end):
            ...
            for w in range(self.frame.width):
                row.append(
                    self._trace_ray(
                        self.eye.corresponding_ray(
                            (self.frame.width, self.frame.height), (w, h)
                        ),
                        0,
                    )
                )
    ...
    def _trace_ray(self, ray, depth, from_inside=False):
        ...
        if least_landa:
            ...
            pos = ray.origin + ray.direction * least_landa
            ...
            for l in self._point_lights:
                ...
                if not self._is_in_shadow(Ray(pos, light_ray_dir), l.position):
                    ...
                    specular_factor = (
                        max(
                            Vector3D.dot(norm.reflect(light_ray_dir), -ray.direction), 0
                        )
                    ...
                    diffuse_factor = (
                        max(Vector3D.dot(light_ray_dir, norm), 0)
                    ...
                    atten = 1.0 / (
                        l.atten_factors[0]
                        + l.atten_factors[1] * dist
                        + l.atten_factors[2] * dist * dist
                    )
